...
const BubbleChart = ({ group_id }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/homebubblechart?group_id=${group_id}`, {
        method: 'GET',
        // headers: {
      ...
      if (!response.ok) {
        setBubbleData(hardcodedData.map(d => ({
          ...d,
  ...
  return (
    <div ref={containerRef} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", width: '100%',backgroundColor:"transparent" }}>
      <div style={{ width: '90%', height: '90%', position: 'relative', margin: '0 auto' }}>
        <svg ref={svgRef}></svg>
      </div>
    </div>
  );
