...
const DoughnutChart = ({ title, groupId }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/graphs/anonymous/anonymousvsidentifieddiscussions?group_id=${groupId}`);
      const result = await response.json();
  
      // Transform API response into the expected format
      const transformedData = {
        labels: ["Anonymous Messages", "Identified Messages"],
        values: [result.anonymous_messages, result.identified_messages],
      };
  
      setData(transformedData); // Set the formatted data
...
const lightenColor = (color, amount) => {
  if (!color || color.length !== 7 || color[0] !== "#") return "#cccccc"; // Fallback to gray if invalid
...
const maxValue = data.values.length > 0 ? Math.max(...data.values) : 1; // Prevent errors if empty
const baseColors = data.values.map(value => value !== undefined ? getColor(value, maxValue) : "#000000"); // Default to black if undefined
const hoverColors = baseColors.map(color => (color ? lightenColor(color, 30) : "#333333")); // Default hover color
