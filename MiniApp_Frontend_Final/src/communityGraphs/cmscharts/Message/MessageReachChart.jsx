...
const LineChart = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('${process.env.REACT_APP_SERVER_URL}/graphs/messages/messageereach?group_id=${group_id}', {
        method: "GET",
    ...
    <ChartContainer className={theme}>
      <ResponsiveContainer width="100%" height="90%">
        ...
        >
          ...
          />
          <YAxis
            label={{ value: 'Message Reach', angle: -90, position: 'insideLeft',style: { fill: "white" , fontSize: "12px"},dy:60, offset: 10 }}
            tick={{ fill: "var(--axis-color)", fontSize: '0.8em' }}
          />
          <Tooltip
