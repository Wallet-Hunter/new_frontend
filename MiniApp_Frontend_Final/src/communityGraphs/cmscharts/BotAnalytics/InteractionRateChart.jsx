  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        '${process.env.REACT_APP_SERVER_URL}/graphs/bots/messages?{group_id}',
        {
          method: "GET",
      ...
      setChartData({
        labels: result.map((item) => item.bot_name),
        datasets: [
          {
            data: result.map((item) => item.message_count),
      ...
      setChartData({
        labels: hardcodedData.map((item) => item.bot_name),
        datasets: [
          {
            data: hardcodedData.map((item) => item.message_count),
            backgroundColor: isDarkMode ? backgroundColorDark : backgroundColorLight,
            borderColor: isDarkMode ? backgroundColorDark : backgroundColorLight,
            borderWidth: 1,
          },
          ...
          options={{
            ...
            plugins: {
              ...
              tooltip: {
                callbacks: {
                  label: (tooltipItem) => {
                    return `Interactions: ${tooltipItem.raw}`; // Tooltip showing number of interactions
                  },
                },
              },
            },
            scales: {
              ...
              y: {
                title: {
                  display: true,
                  text: "Number of Interactions", // Y-axis label
                  color: isDarkMode ? "white" : "black",
                },
                ticks: {
                  color: isDarkMode ? "white" : "black",
...
export default InteractionRateChart;
